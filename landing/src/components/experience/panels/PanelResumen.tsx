  ...
  return (
    <div style={{ background: C.panelSurface, border: `1px solid ${C.panelLine}`, borderRadius: 14, padding: 20 }}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {tasks.map((t, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < tasks.length - 1 ? `1px solid ${C.panelLine}` : "none" }}>
            <div style={{ width: 18, height: 18, borderRadius: 4, border: `1.5px solid ${C.panelLine}`, flexShrink: 0 }} />
      ...
      <div style={{ marginTop: 14, paddingTop: 10, borderTop: `1px solid ${C.panelLine}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, fontSize: 12, color: C.gray }}>
  ...
  return (
    <div style={{ background: C.panelSurface, border: `1px solid ${C.panelLine}`, borderRadius: 14, padding: 20 }}>
      ...
      <div style={{ display: "grid", gap: 6, marginBottom: 16 }}>
        {movs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12.5, padding: "6px 0", borderBottom: i < movs.length - 1 ? `1px solid ${C.panelLine}` : "none" }}>
            <span>{m.name}</span>
            <span style={{ fontWeight: 600, color: m.positive ? C.green : C.red }}>{fmt(m.amount)}</span>
          </div>
        ))}
      </div>
      <div style={{ background: C.panelBg, borderRadius: 10, padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 11, color: C.gray }}>balance del mes</div>
        ...
        {alerts.map((a, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: a.stock === 0 ? C.redBg : C.amberBg, borderRadius: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: C.gray, marginTop: 1 }}>{a.depot} · mínimo: {a.min}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: a.stock === 0 ? C.red : C.amber }}>{a.stock}</div>
              <div style={{ fontSize: 10, color: C.gray }}>unidades</div>
            </div>
          </div>
        ))}
      </div>
...
function WidgetCotizaciones() {
  return (
    <div style={{ background: C.panelSurface, border: `1px solid ${C.panelLine}`, borderRadius: 14, padding: 20 }}>
      ...
      <div style={{ background: C.panelBg, borderRadius: 12, padding: 16, marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div style={{ fontSize: 12, color: C.gray, marginBottom: 4 }}>cotización premium · todos los módulos</div>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
          {["ops","finanzas","stock","cotizador"].map(m => (
            <div key={m} style={{ fontSize: 11, background: C.panelSurface, border: `1px solid ${C.panelLine}`, borderRadius: 6, padding: "3px 10px", color: C.gray, fontWeight: 600 }}>{m}</div>
          ))}
        </div>
      </div>
      <div style={{ display: "flex", gap: 12 }}>
        {[
          { n: "142", label: "enviadas", bg: C.panelBg, color: C.panelInk },
